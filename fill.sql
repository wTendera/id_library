BEGIN;
  ...
  SELECT add_new_borrow(1, 'Jaskółki i Amazonki', 5, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(6, 'Germinal', 7, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(3, 'Targowisko próżności', 1, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(5, 'Opętanie', 2, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(8, 'Opowieść wigilijna', 5, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(2, 'Atlas chmur', 7, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(2, 'Kolor purpury', 2, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(12, 'Okruchy dnia', 1, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(9, 'Pani Bovary', 2, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(21, 'A Fine Balance', 1, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(3, 'Pajęczyna Szarloty', 4, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(7, 'Pięć osób, które spotykamy w niebie', 5, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(12, 'Przygody Sherlocka Holmesa', 7, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(32, 'The Faraway Tree Collection', 1, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(34, 'Jądro ciemności', 2, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(102, 'Mały Książę', 5, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(13, 'Fabryka os', 7, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(57, 'Wodnikowe Wzgórze', 2, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(27, 'Sprzysiężenie głupców', 1, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(10, 'Miasteczko jak Alece Springs', 2, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(22, 'Trzej muszkieterowie', 1, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(33, 'Hamlet', 4, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(34, 'Charlie i fabryka czekolady', 5, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(45, 'Nędznicy', 7, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(67, 'W pustyni i w puszczy', 1, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(122, 'Krzyżacy', 2, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(18, 'Potop', 5, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(90, 'Ogniem i mieczem', 7, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(56, 'Pan Wołodyjowski', 2, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(15, 'Pan Tadeusz, czyli ostatni zajazd na Litwie', 1, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(78, 'Ten obcy', 2, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(111, 'Krew elfów', 1, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(25, 'Wieża jaskółki', 4, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(78, 'Ostatnie życzenie', 5, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(17, 'Pani jeziora', 7, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(98, 'Miecz przeznaczenia', 1, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(56, 'Czas pogardy', 2, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(57, 'Atlas chmur', 7, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(23, 'Kolor purpury', 2, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(11, 'Okruchy dnia', 1, now()-interval '1 week', now()-interval '3 days');
  SELECT add_new_borrow(99, 'Pani Bovary', 2, now()-interval '1 week', now()-interval '3 days');

  SELECT add_new_rating(1, 'Jaskółki i Amazonki', 5);
  SELECT add_new_rating(6, 'Germinal', 7);
  SELECT add_new_rating(3, 'Targowisko próżności', 1);
  SELECT add_new_rating(5, 'Opętanie', 2);
  SELECT add_new_rating(8, 'Opowieść wigilijna', 5);
  SELECT add_new_rating(2, 'Atlas chmur', 7);
  SELECT add_new_rating(2, 'Kolor purpury', 2);
  SELECT add_new_rating(12, 'Okruchy dnia', 1);
  SELECT add_new_rating(9, 'Pani Bovary', 2);
  SELECT add_new_rating(21, 'A Fine Balance', 1);
  SELECT add_new_rating(3, 'Pajęczyna Szarloty', 4);
  SELECT add_new_rating(7, 'Pięć osób, które spotykamy w niebie', 5);
  SELECT add_new_rating(12, 'Przygody Sherlocka Holmesa', 7);
  SELECT add_new_rating(32, 'The Faraway Tree Collection', 1);
  SELECT add_new_rating(34, 'Jądro ciemności', 2);
  SELECT add_new_rating(102, 'Mały Książę', 5);
  SELECT add_new_rating(13, 'Fabryka os', 7);
  SELECT add_new_rating(57, 'Wodnikowe Wzgórze', 2);
  SELECT add_new_rating(27, 'Sprzysiężenie głupców', 1);
  SELECT add_new_rating(10, 'Miasteczko jak Alece Springs', 2);
  SELECT add_new_rating(22, 'Trzej muszkieterowie', 1);
  SELECT add_new_rating(33, 'Hamlet', 4);
  SELECT add_new_rating(34, 'Charlie i fabryka czekolady', 5);
  SELECT add_new_rating(45, 'Nędznicy', 7);
  SELECT add_new_rating(67, 'W pustyni i w puszczy', 1);
  SELECT add_new_rating(122, 'Krzyżacy', 2);
  SELECT add_new_rating(18, 'Potop', 5);
  SELECT add_new_rating(90, 'Ogniem i mieczem', 7);
  SELECT add_new_rating(56, 'Pan Wołodyjowski', 2);
  SELECT add_new_rating(15, 'Pan Tadeusz, czyli ostatni zajazd na Litwie', 1);
  SELECT add_new_rating(78, 'Ten obcy', 2);
  SELECT add_new_rating(111, 'Krew elfów', 1);
  SELECT add_new_rating(25, 'Wieża jaskółki', 4);
  SELECT add_new_rating(78, 'Ostatnie życzenie', 5);
  SELECT add_new_rating(17, 'Pani jeziora', 7);
  SELECT add_new_rating(98, 'Miecz przeznaczenia', 1);
  SELECT add_new_rating(56, 'Czas pogardy', 1);
  SELECT add_new_rating(57, 'Atlas chmur', 3);
  SELECT add_new_rating(23, 'Kolor purpury', 5);
  SELECT add_new_rating(11, 'Okruchy dnia', 2);
  SELECT add_new_rating(99, 'Pani Bovary', 5);
